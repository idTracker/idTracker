% 18-Feb-2014 12:20:58 Anulo la reasignaci�n poco segura (paso 4). Ahora solo
...
% Recorto todo si hay parte sin rellenar (normalmente no deber�a ser
% necesario)
...
% C�lculo de velocidades t�picas
x=mancha2centro(:,:,1);
y=mancha2centro(:,:,2);
vel=[];
...
% Ahora reasigna los que tengan m�s sentido reasignados
...
% identidad asignada. Adem�s anula los que no tengan posibles.
...
%         tipoorden='descend';
%     else
%         tipoorden='ascend'; % Para reasignar, vamos de menos a m�s probables
%     end
...
%             detr�s al camiar la identidad
        ...
        % fije un escal�n por iteraci�n). Adem�s anula los que podr�an
...
% % Ahora quedan los conflictivos. Estos los recorre de menos a m�s probable,
...
% % paso el otro pez habr� cambiado tambi�n su identidad, y podremos
% % reasignarlo.
% sum(fijos)
...
%         mejorpez(1)=NaN; % Si no est� fijo, lo anulamos
%     end
...
%             disp(['Trozo ' num2str(c_trozos) ' reasignado sin m�s'])
%             c_cambiados=c_cambiados+1;
