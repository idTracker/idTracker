% 29-Apr-2014 10:22:16 Elimino encriptaci�n
% 08-Apr-2014 12:42:21 Hago que guarde el mapa promedio y no solo los
% puntos del mapa donde hay informaci�n
% 08-Feb-2014 18:23:44 A�ado el nuevo c�lculo de los centros
% 23-Dec-2013 22:03:48 A�ado dist_max para los mapas
% 12-Dec-2013 17:06:23 Hago que use VideoReader cuando la versi�n de Matlab
% es nueva
% 02-Dec-2013 20:54:44 Arreglo un bug que daba un fallo cuando en un frame
% no aparec�a ning�n bicho.
% 23-Aug-2013 11:39:04 Quito la distinci�n por extensi�n (que en realidad
% ya estaba desactivada)
% 05-Jul-2013 09:49:40 Hago que intente leer los ficheros de golpe aunque
% el v�deo no est� grabado en el laboratorio (comprobando con los primeros
% archivos)
% 18-Jun-2013 19:18:23 A�ado mancha2centro
% 10-Jun-2013 14:41:52 A�ado el log, y quito keyboards
% 01-Jun-2013 10:55:26 A�ado encriptaci�n
...
% 03-Apr-2013 19:20:28 Corrijo un bug que hac�a que fallara cuando en un frame no hab�a ninguna mancha
% 28-Feb-2013 12:02:18 Evito que use rgb2gray cuando el v�deo ya es en blanco y negro
% 27-Feb-2013 15:14:53 Hago que s�lo lea de 1 en 1 cuando los archivos de v�deo no coinciden con los archivos de segm (lo cual indica que viene de un v�deo que no hemos grabado nosotros)
% 19-Feb-2013 12:47:04 Hago que lea el v�deo frame a frame. Es mucho m�s
% lento, pero evita problemas de sincronizaci�n cuando hay frames
% repetidos.
% 26-Jan-2013 11:23:59 A�ado la posibilidad de reutilizar
% 25-Jan-2013 17:32:41 Hago que cargue video de 10 frames en 10 frames,
% para que no sature la memoria
% 25-Jan-2013 12:35:50 Hago que funcione cuando los archivos de v�deo son
% m�s largos que los segms
% 28-Nov-2012 12:23:33 Quito inputs, porque todo est� ya en datosegm. Adem�s anulo usaresta.
% Adem�s meto la interacci�n con el panel.
% 23-Nov-2012 08:46:14 Meto el cambio de contraste en avi2segm
% 12-Nov-2012 17:46:49 Saco de aqu� la adquisici�n del roi, y la mando a datosegm2roi
...
% manchas y no s�lo en la primera.
% 31-Jul-2012 09:30:15 A�ado la posibilidad de excluir una zona para el
% c�lculo de intensmed
% 07-Mar-2012 17:46:29 Corrijo un bug en la l�nea 91, que hab�a corregido
% en el ordenador de Robert.
% 16-Feb-2012 10:39:29 A�ado la posibilidad de invertir el contraste
% 31-Jan-2012 19:59:07 A�ado la posibilidad de roi circular
% 18-Nov-2011 11:03:58 A�ado el roi
% 10-Nov-2011 18:01:05 Cambio avi2miniframes por avi2segm. Esto hace que la
% segmentaci�n en el caso de usar videomedio pase a utilizar exclusivamente
% la diferencia entre el videomedio y el v�deo.
% 09-Nov-2011 14:53:09 Corrijo un error cambiando segm por segm_nue
% 17-Oct-2011 12:19:00 A�ado el c�lculo del solapamiento y los mapas
% 13-Oct-2011 17:25:06 Cambio segm1 a segm_1
% 06-Oct-2011 11:00:13 A�ado n_manchas
% 22-Aug-2011 15:26:51 A�ado background substraction con videomedio
...
npixels=zeros(n_frames,1); % Los ceros marcar�n que no hay mancha
...
%         if strcmpi(datosegm.extension,'avi')
            ...
            %                 disp('Esto no deber�a haber pasado')
            %                 keyboard
            %             end
...
%             inicio=0;
            ...
            %                     end
            %                 end
            %             else % Si el v�deo est� grabado por nosotros, lo carga de una sola vez para ir m�s r�pido
...
%                     fprintf(datosegm.id_log,'r');
                    if size(video_act,3)==3 && isfield(datosegm,'blancoynegro') && ~datosegm.blancoynegro
                        for c_frames=1:size(video_act,4)
                            frame=video_act(:,:,:,c_frames);                            
                            frame=colour_filter(frame);
                            video_act(:,:,1,c_frames)=rgb2gray(frame);%Daniel
                            %video_act(:,:,1,c_frames)=rgb2gray(video_act(:,:,:,c_frames));%Daniel
                        end
                    end
%                     fprintf(datosegm.id_log,',gris');
                ...
                end % if se puede leer el v�deo entero (o no lo sabemos todav�a)
                
                % Comprueba que los frames coinciden cuando los cogemos
                % uno a uno               
                if datosegm.videosbuenos==0 % Si todav�a no se ha comprobado, se comprueba si los v�deos se pueden leer de una vez
                    for cframes_comprueba=equiespaciados(10,length(frames_video))
                        ...
                        if size(frame_act,3)==3 && isfield(datosegm,'blancoynegro') && ~datosegm.blancoynegro                        
                            frame_act=colour_filter(frame_act);
                            frame_act=rgb2gray(frame_act);%Daniel                       
                        end
                    ...
                    if datosegm.videosbuenos<0
                        disp('Parece que hay que leer los v�deos frame a frame')
                    else
                ...
                % Si hace falta, lee el v�deo frame a frame
                if datosegm.videosbuenos<0
                    clear video
                    video=zeros([datosegm.tam 1 length(frames_video)],'uint8');
                    inicio=0;                    
...
%                         fprintf(datosegm.id_log,'%g',c_frames);
                        if size(video_act,3)==3 && isfield(datosegm,'blancoynegro') && ~datosegm.blancoynegro
                            video_act=colour_filter(video_act);
                            video_act(:,:,1)=rgb2gray(video_act);%Daniel
                        end
%                         fprintf(datosegm.id_log,',gris');
                        inicio=inicio+1;
                        video(:,:,:,inicio)=video_act(:,:,1,:);
            ...
            for c_frames=1:length(segm_nue)
                for c_peces=1:length(segm_nue(c_frames).mapas)
                    if segm_nue(c_frames).segmbuena(c_peces)
                        ...
                        else % Si est� saturado, solo actualizamos los que no tengan nada de informacion
