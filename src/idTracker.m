...
%         fprintf(datosegm.id_log,'%s - Start\n',datestr(now,30));
        ...
        if ~isfield(datosegm,'saltatodo') || ~datosegm.saltatodo
            ...
            %     end
            %     if ispc && matlabpool('size')==0 % S�lo abre matlabpool si est� en un pc, porque si es linux indica que estamos en Trueno.
            % %                 disp('Guarning! No se abre matlabpool!')
            %         matlabpool open % Lo abre con la configuraci�n por defecto par el ordenador
            %         %         disp('Guarning! Matlabpool se abre s�lo con 6 procesadores')
...
%             fprintf(datosegm.id_log,'%s - Matlabpool is open\n',datestr(now,30));
