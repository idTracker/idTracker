...
% 29-Apr-2014 10:30:41 Elimino la encriptaci�n
% 07-Apr-2014 10:42:29 A�ado la b�squeda de nueva versi�n
% 04-Feb-2014 15:24:01 Antes de "guarda copia", he hecho que funcione con
% el antigou ginput si la versi�n de matlab es antigua. Adem�s, a�ado
% "soloreferencias" al men� "Advanced"
% 21-Dec-2013 13:21:56 Hago que guarde obj en datos para no tener que
% reabrir el archivo de v�deo si no es necesario
% 12-Dec-2013 15:40:41 Hago que use VideoReader si la versi�n es reciente
% 04-Dec-2013 19:10:48 A�ado h_ocupado para que los callbacks "pidan la
% vez"
% 25-Nov-2013 17:48:15 Paso de trayectorias a trajectories. A�ado lo de
% importar datos de otros v�deos (el 4 de diciembre).
...
% lamentables cambios para que quede m�s serio.
% 20-Nov-2013 08:46:14 Anulo el men� "Advanced" para la versi�n compilada
% del paper.
% 14-Nov-2013 12:15:06 A�ado el men� "Advanced", y preparo para que pueda
% seleccionarse una zona concreta para la normalizaci�n de la intensidad
% 08-Sep-2013 17:04:57 Nada.
% 06-Sep-2013 12:42:21 Hago que el ginput para el ROI vaya de punto en
% punto.
% 25-Aug-2013 19:16:15 Hago que el comportamiento del bot�n ROI cambie
% cuando ya se definido antes algo.
% 04-Jul-2013 20:51:42 A�ado n_procesadores
% 31-May-2013 20:53:26 A�ado encriptaci�n
% 25-Apr-2013 16:51:09 A�ado Trueno
% 19-Apr-2013 17:43:39 Hago que reutilice lo que pueda
% 10-Apr-2013 16:14:23 Evito que falle cuando se pide un frame que se pasa de rango. Adem�s a�ado el intervalo
...
tipos=[1 1 1 1 2 2 1 1 3 2 1]; % 1 significa 'text', 2 significa 'checkbox', 3 significa que es la l�nea de botones del ROI
...
hold(h.ejes,'on')
% h.ocupado se usar� para que las funciones se den la vez. Cada elemento
% corresponde a una funci�n, con el orden
% [actualiza redibuja clicreutiliza calculavideomedio cogezona borramascara importaparametros empiezatracking]
% Cada funci�n s�lo puede empezar cuando h.ocupado es cero para todas las
% dem�s funciones
...
% Comprueba si hay una nueva versi�n
datos.texto_version='';
try    
...
% while any(ocupado)==1 % Si est� ocupado, espera a que se desocupe
%     pause(.1)
%     ocupado=get(h.ocupado,'XData');
%     ocupado(ind_funcion)=0; % Ignora el valor que corresponde a ella misma
% end
...
for c_campos=1:length(datos.campos)
    if isfield(h,datos.campos{c_campos})
        if strcmpi(get(h.(datos.campos{c_campos}),'Style'),'edit')
            ...
            texto(texto=='-')=' '; % Cambia el gui�n por espacio en el intervalo. Luego, str2num interpretar� el espacio como separaci�n entre los dos elementos
...
% Si hace falta, rehace la segmentaci�n
...
elseif any(uno==[h.umbral h.umbral_npixels h.reduceresol h.limpiamierda h.cambiacontraste])    
    inicioborrar=3; % El background puede sobrevivir, s�lo hay que rehacerlo si cambia el ROI.
...
% Controla visibilidad del bot�n para calcular el videomedio
...
if datos.ind_frame~=str2double(get(h.edit_frame,'String'))
    ...
    % Comprueba si hay que crear el objeto v�deo
    crearobj=true;
...
%     datos.cambios.frame=false; 
    % Cambio el orden de los children del colorbar, para que la l�nea siempre quede por encima.
...
% Controla el aspecto del bot�n de ROI
...
reactiva(0,h)
datos=guidata(h.fig);
if uno==h.datosegm % Si no viene del bot�n, estamos simplemente comprobando si hay algo reutilizable con el datosegm que ya tenemos.
