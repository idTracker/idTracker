...
while any(~isnan(mayores))
    ...
    if ~isnan(probtrozos(trozo_act,1))
        ...
        if pezorig~=pez
            fprintf('%g. Trozos left - El trozo %g (frames %g-%g) no se asign� a su mejor opci�n. Probabilidad de original de la asignaci�n = %g. Probabilidad original de la mejor opci�n = %g.\n',sum((~isnan(mayores))),trozo_act,min(frames),max(frames),probtrozos_relac(trozo_act,pez),probtrozos_relac(trozo_act,pezorig))
        end        
...
%             end
            ...
            fprintf('%g. Trozos left - El trozo %g (frames %g-%g) no se asign� por no tener m�ximo claro.\n',sum((~isnan(mayores))),trozo_act,min(frames),max(frames))            
        end % if hay un �nico m�ximo    
        else
            fprintf('%g. Trozos left - El trozo %g (frames %g-%g) no se asign� por ser demasiado incierto. Probabilidad de la identificaci�n: %g (sin contar con otros), %g (contando con otros)\n',sum((~isnan(mayores))),trozo_act,min(frames),max(frames),probtrozos(trozo_act,pez),probtrozos_relac2(trozo_act,pez))            
