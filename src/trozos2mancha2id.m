...
% % disp('GUARNING!! TAL Y COMO EST�, JAM�S CALCULA NUEVAS IDENTIFICACIONES!!')
% while ~isempty(trozosquedan) %&& trozostotales-length(trozosquedan)<800 % HAY QUE QUITAR ESTE L�MITE DE TROZOS   
%     length(trozosquedan)
%     c_trozos=0;
%     c_mapas=0;
...
%     while c_mapas<n_mapasporvuelta && c_trozos<length(trozosquedan)% && trozostotales-length(trozosquedan)<50 % HAY QUE QUITAR ESTE L�MITE DE TROZOS % Acumula 1000 mapas para aprovechar la paralelizaci�n de comparamapas
% %          trozostotales-length(trozosquedan)
%         c_trozos=c_trozos+1;
%         saltatrozo=false;
...
%         % Si el trozo tiene m�s de 1000 frames, usa los primeros 1000. Que
%         % ya est� bien, y si no va a necesitar cargar demasiados segm a la
%         % vez.
%         if nframes_act>1000
%             nframes_act=1000;
...
%                 % Hace la identificaci�n del trozo con las identificaciones que
...
% %                     keyboard
% %                 end
%                 % Comprueba si el trozo est� bien identificado (o todo lo bien que
%                 % se puede)
...
%                     % Va guardando y borrando los archivos que ya no ser�n necesarios
...
%                     end
%                 else
%                     % Si no est� bien identificado, coge los mapas para
%                     % identificarlo
%                     %                 if n_buenos>nframes_min
%                     quedan(trozo_act)=true;
%                     if max(solapos_act)>difminima % Si hay posibilidad de hacerlo bien, coge un cierto n�mero de frames
...
%                     %                 n_buenos=length(buenos);
%                     %                 end
%                     % Va acumulando mapas para calcular los errores. Lo hago as� para aprovechar
%                     % la paralelizaci�n de comparamapas.
...
% %     fprintf('Hecho.\n')
% %     trozosquedan(1:10)
%     % Hace las identificaciones para cada frame, y vuelve a meter la informaci�n en segm
...
%         for c=1:2
% %             try                
%             segmc{arch}(fr).id(manch,ind(c))=segmc{arch}(fr).id(manch,ind(c))+.5; % As� quedar� un 1 en los que los dos mapas se pongan de acuerdo, y 0.5 si no se ponen de acuerdo
% %             catch
% %                 keyboard
% %             end
%         end
